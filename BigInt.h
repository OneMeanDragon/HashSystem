...
namespace COMMON
{
	...
	namespace LARGEMATH {
		...
		public:
			BigInt();
			explicit BigInt(UINT8 input);
			explicit BigInt(UINT16 input);
			explicit BigInt(UINT32 input);
		...
		private:
			...
			explicit BigInt(UINT64 input);
			using bigint_base = UINT32;
			using bigint_extended = UINT64;
