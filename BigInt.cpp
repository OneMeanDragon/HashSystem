...
namespace COMMON
{
	...
	namespace LARGEMATH {
		...
		//why was there uint64 info here when its declared under this opener
		BigInt::BigInt(UINT32 input)
		{
			int i;

			segment_count = sizeof(UINT32) / sizeof(bigint_base);
			segment = (bigint_base*)xmalloc(segment_count * sizeof(bigint_base));
		...
		BigInt::BigInt(UINT64 input)
		{
			int i;
			segment_count = sizeof(UINT64) / sizeof(bigint_base);
			segment = (bigint_base*)xmalloc(segment_count * sizeof(bigint_base));
			for (i = 0; i < segment_count; i++) {
				segment[i] = input & bigint_base_mask;
				input >>= bigint_base_bitcount;
